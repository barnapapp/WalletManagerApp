...
function SpecifyWallet({wallet, funcReference, validateMoneyText}) {

    return (<Grid item lg={6} align={"center"}>
        <Formik initialValues={{name: wallet.name, money: wallet.money, desc: wallet.desc}} onSubmit={async (values, actions) => {
...
function SpecifySharedUsers({searchUser, setCurrentWalletId, wallets, validateUserText, userSearch, users, sharedWalletToUser}) {
...
function AddWallet() {
    ...
    const [setCurrentWalletId, wallet, funcReference, wallets, loadingWallet, currentSharedWallets, deleteSharedWallet, sharedWalletToUser] = useWallet(params.id);
    const [userSearch, users, loading, searchUser] = useUsers();

    if(loading || loadingWallet) return <LinearProgress />
            ...
            <Paper sx={{marginTop: "5em", minHeight: "32em"}}>
                <Grid container maxWidth={"lg"} spacing={2}>
                    ...
                    {currentSharedWallets.map((e, i) => {
                        ...
                        <SpecifyWallet wallet={wallet} funcReference={funcReference} validateMoneyText={validateMoneyText} />
                    <Grid item lg={6} align={"center"}>
                        <SpecifySharedUsers users={users} searchUser={searchUser} setCurrentWalletId={setCurrentWalletId} sharedWalletToUser={sharedWalletToUser} userSearch={userSearch} wallets={wallets} validateUserText={validateUserText} />
                    </Grid>
                </Grid>
            </Paper>
